...
public class UdayaProductSorter implements Comparator<Product> {
    ...
    private int getPositionOfProductInList(String productName) {
        int p1Index = productsPreferablyBoughtAt_Udaya.indexOf(productName);
        if (p1Index == -1) {
            p1Index = productsPreferablyBoughtAt_Udaya.indexOf(productName.concat("s"));
        }
        if ((p1Index == -1) && productName.endsWith("s")) {
            p1Index = productsPreferablyBoughtAt_Udaya.indexOf(productName.substring(0, productName.length() - 1));
        }
        return p1Index;
    }
}