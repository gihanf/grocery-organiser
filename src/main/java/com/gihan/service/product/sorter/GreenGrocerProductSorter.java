...
public class GreenGrocerProductSorter implements Comparator<Product> {
    ...
    private int getPositionOfProductInList(String productName) {
        int p1Index = productsPreferablyBoughtAt_GreenGrocer.indexOf(productName);
        if (p1Index == -1) {
            p1Index = productsPreferablyBoughtAt_GreenGrocer.indexOf(productName.concat("s"));
        }
        if ((p1Index == -1) && productName.endsWith("s")) {
            p1Index = productsPreferablyBoughtAt_GreenGrocer.indexOf(productName.substring(0, productName.length() - 1));
        }
        return p1Index;
    }
}